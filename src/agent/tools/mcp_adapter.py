...
def get_all_agent_tools() -> List[BaseTool]:
    ...
    try:
        ...
        # 加载 datetime 技能
        try:
            datetime_skill = loader.load_skill('datetime')
            tools.extend(datetime_skill.tools)
        except Exception as e:
            print(f"加载 datetime skill 失败: {e}")

        # 加载 calculator 技能
        try:
            calc_skill = loader.load_skill('calculator')
            tools.extend(calc_skill.tools)
        except Exception as e:
            print(f"加载 calculator skill 失败: {e}")
