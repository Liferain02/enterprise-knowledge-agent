...
> 当前参考 HEAD：以当前 Git HEAD 为准；本文是历史收口记录。
> 目标：**以秋招展示与面试可解释性为最高优先级，停止功能堆叠，围绕检索、深度研究模式、知识演化三条核心主线完成最终收口。**
>
> 本文是已完成收口工作的记录，不是新的持续开发清单。若旧 `goal*.md`、历史 P0～P6 文档或旧路线与本文冲突，以当前代码、评测说明和文档总览为准。

> 当前状态（2026-09-07）：本文规划的核心收口已完成。最终检索与知识演化结果见《../03-评测与验证/02-秋招版评测收口说明.md》；没有新的失败证据前，项目不再增加智能体、基础设施或新的检索策略。
...
1. 不再追求功能数量。
2. 不增加智能体，不重写 LangGraph 主图。
3. 每个改动必须能回答：为什么做、解决什么问题、如何评测、是否值得保留。
4. 没有失败归因或基准测试证据，不实现新能力。
5. 不为了让实验更漂亮去修改冻结数据、Gold、评分器、阈值或统计口径。
...
## 2.1 独立问题 Multi-turn Rewrite

已有：

- 冻结数据集 24 条；
- Trigger Precision / 召回率 / F1 = 1.000 / 1.000 / 1.000；
- 正例 Hit@5：0.867 → 1.000；
- 正例 MRR@5：0.867 → 0.933；
- ACL violation = 0；
- Positive 回归测试 Rate = 0；
- 真实 Chromium 多轮验收 8/8 路由正确、8/8 Gold 来源命中。
...
## 3.3 条件性借鉴：technical 分词器 / Parent-Child
...
若 stable doc_id → 当前 元数据 需要重构整个 文档 Registry：
...
# 7. P3：只有基准测试暴露问题时才做检索 POC

## P3-A Technical-aware BM25

仅当 technical exact 词元子集明显差时做。
...
- 已有普通模式 / 深度研究模式（带门槛控制）/ 深度研究模式（完整修订）；
- 已有 V3 声明-level Eval；
- 已有可执行性门槛 rollback；
- 已有质量 / 成本边界。
...
# 10. Selective 深度研究模式 Routing 停止上线
...
> 已评估选择性深度研究模式，真实回答收益不足以覆盖延迟和引用支持风险，因此保留用户显式开启。
...
最终文档入口为 [`docs/00-总览/00-文档总览.md`](00-文档总览.md)。根目录只保留 README；最终收口任务书、评测说明、源码讲解和面试资料统一放在 `docs/`。已完成 Goal、阶段计划和旧路线不再单独维护，避免被误当成待办。
...
> 构建 Vector + BM25 + RRF + 重排的 ACL-aware 混合检索 RAG，并针对多轮省略/指代实现选择性独立问题改写；建立冻结检索基准测试，从 Hit@1/Hit@5、MRR、NDCG、漂移率和 ACL 违规等维度评估检索稳定性，最终填写真实实验数据。

## Bullet 2：深度研究模式

> 基于 LangGraph 设计资料研究员 → 分析员 → 审核员有界深度研究模式，将检索证据结构化为证据 / 声明 / Review，并限制最多一次修订；构建声明级冻结因果评测，量化声明 F1、Faithfulness、延迟和 Token，识别深度研究模式的真实适用边界。

## Bullet 3：Memory / 知识演化

> 设计“证据支撑的记忆”与项目知识沉淀闭环：审核员通过的事实经用户确认写入 Mem0，正式科研结论经独立发布动作进入项目知识，并保留研究运行记录、声明和证据来源，支持撤销、替代和访问控制复核，避免模型回答自动污染长期知识。

## Bullet 4：可选工程稳定性

> 通过真实 Chromium 完成多轮检索、研究运行记录与跨会话 Memory E2E 验收，8 组冻结多轮样本路由与 Gold 来源命中均为 8/8，并修复 SSE 分片、Mem0 生命周期和结构化输出等真实链路问题。
